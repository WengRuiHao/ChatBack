...
//		try {
//			sendMessage(service, "me", createEmail("usually12336@gmail.com", "忘記密碼，提供修改密碼的url", "信件內容"));
...
//    	clearStoredTokens();
		...
		GoogleAuthorizationCodeFlow flow = new GoogleAuthorizationCodeFlow.Builder(
				...
				.setAccessType("offline").setDataStoreFactory(new FileDataStoreFactory(new java.io.File("token"))) // 設定存儲資料夾
				.build();	
...
//        email.setFrom(new InternetAddress(from));
	...
	public static void clearStoredTokens() throws IOException {
		Path tokensPath = Paths.get("token");
		if (Files.exists(tokensPath)) {
			Files.walk(tokensPath).map(Path::toFile).forEach(File::delete);
		}
	}
}